...
import { checkBillingStatus, loginCommand, requestCard, type BillingStatus } from './auth.ts';
import {  isLoggedIn } from './sessions.ts';
import { renderBlacdiskLogo } from "./maskot.ts";
import { runBlacDiskPipeline } from "./pipe.ts";
import { ensureAnthropicApiKey } from "./key.ts";
...
function askForPrompt(status:BillingStatus | null): Promise<string> {
  return new Promise(async (resolve, reject) => {
    ...
    rl.question("\x1b[1m\x1b[34m> \x1b[0m", (prompt) => {
      ...
      if(prompt.length>0)
        resolve(prompt);
      else reject(new Error("Type a prompt before running"))
    });
  });
}
...
.action(async () => {
  try {
    ...
    if (!startBlacdisk && !isSignedIn) process.exit(0);

    await ensureAnthropicApiKey();

    const prompt : string = await askForPrompt(effectiveStatus);
