...
function box(title) {
    ...
    console.log(chalk.gray('│ ') + chalk.bold.cyan(title) + ' '.repeat(pad) + chalk.gray(' │'));
    console.log(chalk.gray('└' + '─'.repeat(WIDTH - 2) + '┘'));
}
function renderModelMenu() {
    ...
    for (const m of MODELS) {
        ...
        console.log(`  ${chalk.yellow(m.key)}  ${chalk.white(m.label.padEnd(20))} ${chalk.dim(m.tag)}${marker}`);
    }
    console.log(rule());
}
function renderEffortMenu() {
    ...
    for (const e of EFFORT_LEVELS) {
        ...
        console.log(`  ${chalk.yellow(e.key)}  ${chalk.white(e.label.padEnd(10))} ${chalk.dim(e.blurb)}${marker}`);
    }
    console.log(rule());
}
function renderSummary(modelTag, effortTag) {
...
function createFilteredStdin(source) {
...
async function configureClaudeSession() {
    ...
    // 1. Model selection
    renderModelMenu();
    const modelAnswer = (await rl.question(chalk.yellow(`  Choice [${DEFAULT_MODEL_KEY}]: `))).trim();
    const chosenModel = MODELS.find((m) => m.key === modelAnswer) ??
        MODELS.find((m) => m.key === DEFAULT_MODEL_KEY);
    // 2. Effort selection
    renderEffortMenu();
    const effortAnswer = (await rl.question(chalk.yellow(`  Choice [${DEFAULT_EFFORT_KEY}]: `))).trim();
    const chosenEffort = EFFORT_LEVELS.find((e) => e.key === effortAnswer) ??
...
//# sourceMappingURL=configureClaudeSession.js.map